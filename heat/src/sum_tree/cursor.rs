    ...
    U: Dimension<'a, T::Summary>
{
    ...
    pub fn item(&self) -> Option<&'a T> {
        assert!(self.did_seek, "deve procurar antes de chamar este método");
                    ...
                    match *entry.tree.0 {
                        ...
                        Node::Leaf {
                                ...
                                .zip(&item_summaries[entry.index..])
                            {
                                ...
                                if comparison == Ordering::Greater || (comparison == Ordering::Equal && bias == SeekBias::Right) {
                                    self.seek_dimension.add_summary(item_summary);
                                    self.sum_dimension.add_summary(item_summary);
            ...
            loop {
                ...
                match *subtree.0 {
                    Node::Internal {
                        ...
                        for (index, (child_tree, child_summary)) in child_trees.iter().zip(child_summaries).enumerate() {
                            let mut child_end = self.seek_dimension.clone();
                    ...
                    Node::Leaf {
                        ...
                        for (index, (item, item_summary)) in items.iter().zip(item_summaries).enumerate() {
                            let mut child_end = self.seek_dimension.clone();
