...
impl<T: Item> Node<T> {
    ...
    fn child_trees(&self) -> &ArrayVec<[SumTree<T>; 2 * TREE_BASE]> {
        match self {
            ...
            Node::Leaf { .. } => panic!("os nós folha não têm árvores filhas")
        }
    }
    ...
    fn items(&self) -> &ArrayVec<[T; 2 * TREE_BASE]> {
        match self {
            ...
            Node::Internal { .. } => panic!("nodes internos não possuem itens")
        }
    }
...
mod tests {
    ...
    fn test_cursor() {
        ...
        assert_eq!(cursor.slice(&Count(0), SeekBias::Right).items(), Vec::<u8>::new());
        assert_eq!(cursor.item(), None);
        assert_eq!(cursor.prev_item(), None);
        assert_eq!(cursor.start(), &Sum(0));

        // árvore de elemento único
        let mut tree = SumTree::<u8>::new();
        tree.extend(vec![1]);
        let mut cursor = tree.cursor::<Count, Sum>();
        assert_eq!(cursor.slice(&Count(0), SeekBias::Right).items(), Vec::<u8>::new());
        ...
        assert_eq!(cursor.slice(&tree.extent::<Count>(), SeekBias::Right).items(), [1]);
        assert_eq!(cursor.item(), None);
        assert_eq!(cursor.prev_item(), Some(&1));
        assert_eq!(cursor.start(), &Sum(1));

        // árvore múltiplo-elementos
        ...
        let mut cursor = tree.cursor::<Count, Sum>();
        assert_eq!(cursor.slice(&tree.extent::<Count>(), SeekBias::Right).items(), tree.items());
        assert_eq!(cursor.item(), None);
        assert_eq!(cursor.prev_item(), Some(&6));
        assert_eq!(cursor.start(), &Sum(21));

        cursor.seek(&Count(3), SeekBias::Right);
        assert_eq!(cursor.slice(&tree.extent::<Count>(), SeekBias::Right).items(), [4, 5, 6]);
        assert_eq!(cursor.item(), None);
        assert_eq!(cursor.prev_item(), Some(&6));
        assert_eq!(cursor.start(), &Sum(21));

        // a busca pode inclinar-se para a esquerda ou para a direita
        cursor.seek(&Count(1), SeekBias::Left);
        assert_eq!(cursor.item(), Some(&1));
        cursor.seek(&Count(1), SeekBias::Right);
        assert_eq!(cursor.item(), Some(&2));

        // o fatiamento sem redefinição começa onde o cursor está estacionado
        cursor.seek(&Count(1), SeekBias::Right);
