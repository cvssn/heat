...
fn open_paths(params: &OpenParams, app: &mut MutableAppContext) {
    log::info!("caminhos abertos {:?}", params.paths);

    // abre caminhos no espaço de trabalho existente, se possível
        ...
        if let Some(handle) = app.root_view::<WorkspaceView>(window_id) {
            if handle.update(app, |view, ctx| {
                if view.contains_paths(&params.paths, ctx.app()) {
                    ...
                    log::info!("abrir caminhos no espaço de trabalho existente");
    ...
    log::info!("abrir novo espaço de trabalho");

    // adiciona um novo espaço de trabalho, se necessário
