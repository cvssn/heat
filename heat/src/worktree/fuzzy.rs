...
pub fn match_paths(
    ...
    let path_count = paths_by_tree_id.iter().fold(0, |sum, (_, _, paths)| sum + paths.len());
        ...
        if query_char == path_char || (is_path_sep && query_char == '_' || query_char == '\\') {
            ...
            if j > path_idx {
                ...
                if last == '/' {
                    ...
                    char_score = MIN_DISTANCE_PENALTY.max(BASE_DISTANCE_PENALTY - (j - path_idx - 1) as f64 * ADDITIONAL_DISTANCE_PENALTY);
                }
            }

            // aplicar uma penalidade severa se o caso não corresponder
            //
            // isso fará com que as correspondências exatas tenham pontuação mais alta
            // do que as que não diferenciam maiúsculas de minúsculas e o caminho bata
            ...
            // dimensiona a pontuação com base na profundidade do patch em que
            ...
            if min_score > 0.0 {
                ...
                // as pontuações apenas diminuem
                //
                // se não conseguirmos passar o melhor anterior, fiança
                if next_score < min_score {
                    // certificar que a pontuação seja diferente de zero para que possamos usá-la na tabela de memorandos
                    if score == 0.0 {
                        score = 1e-18;
                    }
            ...
            if new_score > score {
                ...
                // otimização: não pode marcar melhor que 1
                if new_score == 1.0 {
                    break;
                }
            }
