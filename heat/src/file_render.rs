    ...
    fn render_matches(&self) -> Box<dyn Element> {
        if self.matches.is_empty() {
            ...
            return Container::new(
                Label::new(
                    "sem combinações".into(),

                    settings.ui_font_family,
                    settings.ui_font_size
                ).boxed()
            ).with_margin_top(6.0).boxed();
        }
    ...
    fn render_match(
        ...
        self.worktree(tree_id, app).map(|tree| {
            ...
            if index == self.selected || index < self.matches.len() - 1 {
                container = container.with_border(Border::bottom(1.0, ColorU::new(0xdb, 0xdb, 0xdc, 0xff)));
            }
...
mod tests {
    ...
    fn test_matching_paths() -> Result<()> {
        App::test((), |mut app| async move {
            ...
            let (window_id, workspace_view) = app.add_window(|ctx| WorkspaceView::new(workspace.clone(), settings, ctx));

            app.finish_pending_tasks().await; // abrir e preencher a árvore de trabalho
            ...
            app.finish_pending_tasks().await; // pesquisa de caminho completo
