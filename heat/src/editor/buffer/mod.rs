...
impl Edit {
    pub fn delta(&self) -> isize {
        (self.new_range.end - self.new_range.start) as isize - (self.old_range.end - self.old_range.start) as isize
    }
...
impl Buffer {
    ...
    fn build(replica_id: ReplicaId, file: Option<FileHandle>, base_text: String) -> Self {
        ...
        if base_insertion.text.len() > 0 {
            let base_fragment_id = FragmentId::between(&FragmentId::min_value(), &FragmentId::max_value());
    ...
    pub fn simulate_typing<T: Rng>(&mut self, rng: &mut T) {
        ...
        for char in new_text.chars() {
            self.edit(Some(range.clone()), char.to_string().as_str(), None).unwrap();
        ...
        I: IntoIterator<Item = Range<Point>>
    {
        ...
        self.selections.insert(lamport_timestamp, selections.clone());
        ...
        I: IntoIterator<Item = Range<Point>>,
    {
        self.selections
            .remove(&set_id)
            .ok_or_else(|| anyhow!("id do conjunto de seleção inválido {:?}", set_id))?;
    ...
    fn apply_edit(
        ...
        let mut new_fragments = cursor.slice(&FragmentIdRef::new(&start_fragment_id), SeekBias::Left);
    ...
    fn resolve_fragment_id(&self, edit_id: time::Local, offset: usize) -> Result<FragmentId> {
        let split_tree = self
            .insertion_splits
            .get(&edit_id)
            .ok_or_else(|| anyhow!("operação inválida"))?;
        ...
        I: Iterator<Item = Range<usize>>
    {
        ...
        while cur_range.is_some() && cursor.item().is_some() {
            ...
            // encontrar todos os splices que começam ou terminam no fragmento atual. em seguida, dividir
            // o fragmento e remontar nas duas árvores, contando o texto excluído e o texto inserido recentemente
            while cur_range.as_ref().map_or(false, |r| r.start < fragment_end) {
                ...
                // se o splice terminar dentro deste fragmento, podemos avançar para o próximo splice e
                // verificar se ela também intercepta o fragmento atual. caso contrário, saímos do loop
                // e encontramos o primeiro fragmento que o splice não contém totalmente
                if range.end <= fragment_end {
            ...
            new_split_tree.push_tree(splits_cursor.slice(&old_split_tree.extent::<usize>(), SeekBias::Right));

            self.insertion_splits.insert(fragment.insertion.id, new_split_tree);

            new_fragments.push(fragment);

            // avançar até encontrar um fragmento que não esteja totalmente contido pelo splice atual
            cursor.next();
            ...
            if let Some(range) = cur_range.clone() {
                ...
                // se o splice que está avaliando atualmente começar após o final do fragmento em que o cursor está
                // estacionado, devemos procurar o intervalo inicial do próximo splice e empurrar todos os fragmentos
                // intermediários para a nova árvore
        ...
        // intervalo de identificador que está no final do buffer, se existir
        //
        // nunca deverá ter múltiplos porque os intervalos devem ser disjuntos
        if cur_range.is_some() {
    ...
    fn split_fragment(
        ...
        if range.end == fragment.start_offset() {
            ...
            cursor.next();
            new_split_tree.push_tree(cursor.slice(&old_split_tree.extent::<usize>(), SeekBias::Right));
    ...
    pub fn anchor_at<T: ToOffset>(&self, position: T, bias: AnchorBias) -> Result<Anchor> {
        ...
        if offset > max_offset {
            return Err(anyhow!("offset está fora de alcance"));
        }
        ...
        match anchor {
            ...
            Anchor::Middle {
                ...
                let splits = self
                    .insertion_splits
                    .get(&insertion_id)
                    .ok_or_else(|| anyhow!("split não existente para id de inserção"))?;
                ...
                splits_cursor
                    .item()
                    .ok_or_else(|| anyhow!("offset dividido está fora de alcance"))
        ...
        match anchor {
            ...
            Anchor::Middle {
                ...
                let fragment = fragments_cursor
                    .item()
                    .ok_or_else(|| anyhow!("o id de fragmento não existe"))?;
    ...
    pub fn point_for_offset(&self, offset: usize) -> Result<Point> {
        ...
        fragments_cursor
            .item()
            .ok_or_else(|| anyhow!("offset está fora de alcance"))
            .map(|fragment| {
...
impl ToOffset for Point {
    fn to_offset(&self, buffer: &Buffer) -> Result<usize> {
        ...
        fragments_cursor
            .item()
            .ok_or_else(|| anyhow!("ponto está fora de alcance"))
            .map(|fragment| {
                let overshoot = fragment
...
mod tests {
    ...
    fn test_anchors() -> Result<()> {
        ...
        // certificar que a ancoragem em um ponto seja equivalente à ancoragem em um deslocamento
        assert_eq!(
    ...
    impl Buffer {
            ...
            T: Rng
        {
            ...
            // adição aleatória, remover ou mutar sets de seleção
