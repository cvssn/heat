                ...
                _ => {
                    if let Some(component) = components.peek() {
                        if component.is_empty() && source.ends_with('-') {
                            ...
                            return Err(anyhow!("keystroke inválido `{}`", source));
                        }
                    } else {
...
impl ContextPredicate {
    ...
    fn from_node(node: Node, source: &[u8]) -> anyhow::Result<Self> {
        let parse_error = "erro ao analisar o predicado do contexto";
        let kind = node.kind();
...
mod tests {
    ...
    fn test_matcher() -> anyhow::Result<()> {
        ...
        // as correspondências com falha não interferem na correspondência das chaves subsequentes
        ...
        // as teclas pendentes são apagadas quando o contexto muda
        assert_eq!(matcher.test_keystroke::<()>("a", 1, &ctx_b), None);
        ...
        // as teclas pendentes são mantidas por visualização
        assert_eq!(matcher.test_keystroke::<()>("a", 1, &ctx_b), None);
