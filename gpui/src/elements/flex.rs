    ...
    fn layout(
        ...
        for child in &mut self.children {
            if let Some(flex) = Self::child_flex(child.as_ref()) {
                ...
                let child_constraint = SizeConstraint::strict_along(cross_axis, constraint.max_along(cross_axis));
        ...
        let mut size = if total_flex > 0.0 {
            if constraint.max_along(self.axis).is_infinite() {
                panic!("flex contém children flexível porém contém um constraint infinito no axis flex");
            }
