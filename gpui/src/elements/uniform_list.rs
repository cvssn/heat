    ...
    F: Fn(Range<usize>, &mut Vec<Box<dyn Element>>, &AppContext)
{
    fn layout(
        ...
        if constraint.max.y().is_infinite() {
            unimplemented!(
                "uniformlist não suporta renderização com altura irrestrita"
            );
        }

        let mut size = constraint.max;
        let mut item_constraint = SizeConstraint::new(vec2f(size.x(), 0.0), vec2f(size.x(), f32::INFINITY));
        ...
        if let Some(first_item) = self.items.first_mut() {
            ...
            let start = cmp::min((self.scroll_top() / item_size.y()) as usize, self.item_count);
